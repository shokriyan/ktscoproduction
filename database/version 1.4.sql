...
CREATE TABLE `BANKDEPOSIT` (
  ...
  `received_RECEIVE_ID` int(11) DEFAULT NULL,
  `banks_bank_id` int(11) NOT NULL,
  `employee_employee_id` int(11) DEFAULT NULL,
  `banks_fromBank` int(11) DEFAULT NULL,
...
--
-- Temporary view structure for view `totalpaiddolor`
--

DROP TABLE IF EXISTS `totalpaiddolor`;
/*!50001 DROP VIEW IF EXISTS `totalpaiddolor`*/;
SET @saved_cs_client     = @@character_set_client;
SET character_set_client = utf8mb4;
/*!50001 CREATE VIEW `totalpaiddolor` AS SELECT 
 1 AS `expns_id`,
 1 AS `paiddoloramount`*/;
SET character_set_client = @saved_cs_client;
...
--
-- Temporary view structure for view `usdreceivedtotal`
--

DROP TABLE IF EXISTS `usdreceivedtotal`;
/*!50001 DROP VIEW IF EXISTS `usdreceivedtotal`*/;
SET @saved_cs_client     = @@character_set_client;
SET character_set_client = utf8mb4;
/*!50001 CREATE VIEW `usdreceivedtotal` AS SELECT 
 1 AS `bill_id`,
 1 AS `receivedTotal`*/;
SET character_set_client = @saved_cs_client;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `bankreceivetotalview` AS select `bankdeposit`.`banks_bank_id` AS `banks_bank_id`,sum(`bankdeposit`.`AMOUNT`) AS `totalAmount`,ifnull((select sum(`paybills`.`amount`) from `paybills` where (`paybills`.`account_id` = `bankdeposit`.`banks_bank_id`)),0) AS `totalPaid`,(sum(`bankdeposit`.`AMOUNT`) - ifnull((select sum(`paybills`.`amount`) from `paybills` where (`paybills`.`account_id` = `bankdeposit`.`banks_bank_id`)),0)) AS `accountBalance` from `bankdeposit` group by `bankdeposit`.`banks_bank_id` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `productiontotalview` AS select `prod`.`prod_id` AS `prod_id`,`prod`.`prod_name` AS `prod_name`,`prod`.`prod_um` AS `prod_um`,ifnull(sum(`prodDetail`.`quantity`),0) AS `productionTotal` from (`products` `prod` left join `productiondetail` `prodDetail` on((`prod`.`prod_id` = `prodDetail`.`product_id`))) where (`prod`.`factory_prod` = 1) group by `prod`.`prod_id` order by `prod`.`prod_id` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
--
-- Final view structure for view `totalpaiddolor`
--
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `totalpaiddolor` AS select `pb`.`expns_id` AS `expns_id`,sum((`pb`.`amount` * `cu`.`rate`)) AS `paiddoloramount` from (`paybills` `pb` join `currencies` `cu` on((`cu`.`currency` = (select `expensebill`.`currency` from `expensebill` where (`expensebill`.`expns_id` = `pb`.`expns_id`))))) where (`cu`.`entryDate` = `pb`.`paydate`) group by `pb`.`expns_id` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
--
-- Final view structure for view `usdreceivedtotal`
--
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `usdreceivedtotal` AS select `r`.`bill_id` AS `bill_id`,sum((`r`.`amount` * `c`.`rate`)) AS `receivedTotal` from ((`received` `r` join `salesbilltotal` `sb` on((`sb`.`bill_id` = `r`.`bill_id`))) join `currencies` `c` on((`sb`.`currencyType` = `c`.`currency`))) where (`r`.`receive_date` = `c`.`entryDate`) group by `r`.`bill_id` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2020-03-14 17:57:48
